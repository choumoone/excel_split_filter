...
class ExcelProcessor:
    ...
    def add_filter_group(self):
        ...
        self.filter_groups.append([])  # 添加一个新的筛选组
    ...
    def add_filter_row(self, group_frame):
        ...
        operation_cb = ttk.Combobox(row_frame, values=["=", ">", "<", ">=", "<=", "包含", "不包含", "非空", "为空"])
        operation_cb.pack(side='left', padx=5)

        value_entry = ttk.Entry(row_frame)
        value_entry.pack(side='left', padx=5)

        remove_button = tk.Button(row_frame, text="移除", command=lambda: self.remove_filter_row(row_frame, group_frame))
